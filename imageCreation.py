...
for (nx, ny, nw, nh) in nose:
    ...
    overlay_image_alpha(image, moustache_resized, nx, ny + int(nh/1.5), moustache_resized[:, :, 3])
    break

for (ex, ey, ew, eh) in eyes:
    sunglasses_resized = cv2.resize(sunglasses, (ew * 2, eh))
    overlay_image_alpha(image, sunglasses_resized, ex - ew // 2, ey, sunglasses_resized[:, :, 3])
    break
